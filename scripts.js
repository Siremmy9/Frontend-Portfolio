/* ============================================================
   EMMANUEL JAMES — PORTFOLIO SCRIPTS
   ============================================================ */
...
function filterProjects() {
  ...
  projectCards.forEach(card => {
    ...
    const matchSearch = !searchQuery || name.includes(searchQuery.toLowerCase()) || tags.includes(searchQuery.toLowerCase());
...
track.addEventListener('touchend', e => {
  ...
  if (Math.abs(delta) > 50) goToSlide(delta < 0 ? currentSlide + 1 : currentSlide - 1);
}, { passive: true });
...
function validateForm() {
  ...
  if (!name) { showError('contact-name', 'name-error', 'Please enter your full name.'); valid = false; }
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    showError('contact-email', 'email-error', 'Please enter a valid email address.');
    valid = false;
  }
  if (!subject) { showError('contact-subject', 'subject-error', 'Please enter a subject.'); valid = false; }
  if (message.length < 15) {
    showError('contact-message', 'message-error', 'Message must be at least 15 characters.');
    valid = false;
  }
